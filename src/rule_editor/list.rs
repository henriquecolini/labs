...
impl List {
    ...
    pub fn show<T, R>(self, ui: &mut Ui, iter: impl Iterator<Item = T>, mut add_body: impl FnMut(&mut Ui, T) -> R) -> Option<VecAction> {
        let mut action = None;
        ui.group(|ui| {
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
    ...
    pub fn show_vec<T, R>(self, ui: &mut Ui, vec: &mut Vec<T>, new_item: impl Fn() -> T, add_body: impl FnMut(&mut Ui, (usize, &mut T)) -> R) {
        self.show(ui, vec.iter_mut().enumerate(), add_body).apply(vec, new_item);
    }
    pub fn show_vec_default<T: Default, R>(self, ui: &mut Ui, vec: &mut Vec<T>, add_body: impl FnMut(&mut Ui, (usize, &mut T)) -> R) {
        self.show(ui, vec.iter_mut().enumerate(), add_body).apply_default(vec);
    }
}

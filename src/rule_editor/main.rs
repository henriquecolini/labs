...
impl RulesApp {
    fn new() -> Self {
        let (mut grades, mut teachers, mut subjects, mut times): (Vec<_>, Vec<_>, Vec<_>, Vec<_>) 
        = load_school(SCHOOL_PATH)
            .map(|school| (
                school.grades().map(|grade| grade.name.to_owned()).collect(),
                school.teachers().map(|teacher| teacher.name.to_owned()).collect(),
                school.subjects().map(|subject| subject.name.to_owned()).collect(),
                school.slots().map(|s| s.time).collect::<HashSet<_>>().into_iter().collect()),
            ).unwrap_or_default();
        grades.sort();
        teachers.sort();
        subjects.sort();
...
impl eframe::App for RulesApp {
    fn update(&mut self, ctx: &egui::Context, _: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top_panel").min_height(40.).show(ctx, |ui| {
                    ...
                    match last {
                        Ok(last) => {
                            ...
                            if elapsed < Duration::from_secs(20) {
                                ...
                                ui.label(format!("Salvo há {}s", (elapsed.as_secs()/20)*20));
                            } else {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                List::new("Laboratório").wide().show_vec_default(ui, &mut self.rules.labs, |ui, (lab_idx, lab)| {
                    ui.vertical(|ui| {
                        ui.horizontal(|ui| {
                            ui.label("Lab.");
                        ...
                        List::new("Matéria").show_vec_default(ui, &mut lab.classes, |ui, (class_idx, class)| {
                            ui.horizontal(|ui| {
                                ui.horizontal(|ui| {
                                    ui.label("Mat.");
                                ...
                                List::new("Professor").show_vec_default(ui, &mut class.teachers, |ui, (teacher_idx, teacher)| {
                                    ui.horizontal(|ui| {
                                        ui.label("Prof.");
                        ...
                        List::new("Horário Proibido").show_vec(
                            ...
                            || self.times.first().map(|t| *t).unwrap_or_default(), |ui, (time_idx, time)| {
                                egui::ComboBox::from_id_salt(format!("time_{lab_idx}_{time_idx}"))
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native("Editor de Regras", options, Box::new(|_cc| Ok(Box::new(RulesApp::new()))))
}
